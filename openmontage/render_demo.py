...
def render_demo(name: str, props_path: Path, npx_cmd: str) -> None:
    ...
    # make headless Chromium reject remote fonts with ERR_CERT_AUTHORITY_INVALID.
    # Off by default; enable with `export REMOTION_IGNORE_CERT_ERRORS=1`.
    if os.environ.get("REMOTION_IGNORE_CERT_ERRORS") == "1":
        render_cmd.extend(["--ignore-certificate-errors", "--disable-web-security"])
