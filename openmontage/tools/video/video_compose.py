...
class VideoCompose(BaseTool):
    ...
    def _remotion_render(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        # Sandbox escape hatch: proxied-TLS environments (e.g. Claude Code web)
        # make headless Chromium reject remote assets (fonts) with
        # ERR_CERT_AUTHORITY_INVALID, aborting the render. Off by default;
        # enable with `export REMOTION_IGNORE_CERT_ERRORS=1` on trusted networks.
        if os.environ.get("REMOTION_IGNORE_CERT_ERRORS") == "1":
            cmd.extend(["--ignore-certificate-errors", "--disable-web-security"])
